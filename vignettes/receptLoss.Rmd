...
- There are ~50 NHRs, several of which (estrogen, progesterone, 
and androgen receptor) are utilized as biomarkers in cancer.
But could there be there more?
...
and neglect expression levels in normal tissue.
...
* `muAb` ($\mu_A$) - "mu above", the arithmetic mean across expression values from tumors greater than
(ie above) the `lowerBound`.

* `muBl`($\mu_B$) - "mu below", the arithmetic mean across expression values from tumors less than
...
progesterone, and androgen receptors) in uterine tumors
...
The code for generating `nhrs` is available in ["data-raw/nhrs.R"](
...
the [Cancer Genome Atlas/Genomic Data Commons](https://portal.gdc.cancer.gov/projects/TCGA-UCEC). 
