...
`receptLoss` is an R package that detects potentially novel tumor suppressor 
(TS) genes using RNA expression data from tumor and adjacent normal tissue. 
Most TS have been identified using DNA mutations, but no systematic approach
exists for identifying novel TS based on patterns of RNA expression.

`receptLoss` is focused on identifying candidate TS among the 
nuclear hormone receptors, a family 
of genes that has known roles in oncogenesis and whose loss 
of expression has been correlated with poor outcomes in endometrial and 
breast cancer. 

`receptLoss` is complementary to 
[`oncomix`](https://bioconductor.org/packages/release/bioc/html/oncomix.html). Whereas `oncomix` detects genes that **gain** expression in subsets of tumors relative to normal 
tissue, `receptLoss` detects genes that **lose** expression in subsets 
...
`meetsMinPropPerGrp` (either TRUE or FALSE) in the output, explained below.
...
* `lowerBound` - the lower bound, or the value `nSdBelow` the mean of
the normal tissue expression data.

* `propTumLessThBound` - the proportion of tumor samples with
expression levels less than `lowerBound`. Indicated by $\pi_L$ (also see visual),
...
* `muAb` - "mu above", the mean expression value of tumors greater than
(ie above) the `lowerBound`.

* `muBl` - "mu below", the mean expression value of tumors less than
(ie below) the `lowerBound`.

* `deltaMu` - the difference between `muAb` and `muBl`. The rows are 
...
constitute a true bimodal distribution.
...
If `minPropPerGroup` $< min(\pi_L, 1-\pi_L)$, 
...
We take the top-ranked gene and plot its distribution.
```{r}
clrs <- c("#E78AC3", "#8DA0CB")
plotReceptLoss(exprMatrNml, exprMatrTum, rl, geneName=as.character(rl[1,1]), clrs=clrs)
```
...
* The dotted curve represents a Gaussian distribution over the 
the expression data from normal tissue,
...
expression data from normal tissue. The position of this line 
was derived from the value of `nSdBelow` 
(in this example, `r nSdBelow`) and the mean of the expression 
from normal tissue. 
...
expression relative to normal tissue. Thus, the pink line forms 
...
This object facilitates filtering of NHRs from a large matrix of gene 
expression data, as it contains various gene identifiers 
(e.g. HGNC symbol and ID, Entrez ID, and Ensembl ID) that might
be obtained from different RNA-seq datasets.
...
that rely on solely on expression patterns in tumor tissue.

    - Biologically-relevant tumor subclassification is 
    important in tumor biomarker development and in the 
    development of new anti-cancer therapeutics. 

- `receptLoss` can be expanded for use with a variety of tumor types,
biological data types (e.g. miRNA, protein expression), and even 
...
tumor samples (hundreds) relative to normal samples (dozens). We make the
assumption that the normal expression data are distributed as a
single Gaussian, which helps us draw statistical conclusions from small numbers.
